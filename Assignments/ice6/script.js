...
document.getElementById("searchPokemon").addEventListener("click", async () => {
    ...
    let pokemonName = userInput.value.trim();
    try{
        ...
        document.getElementById("pokemon-details").innerText = "Super unexpected error occured. Please try again :("
    }
...
function displayPokemonDetails(pokemon) {
    let detailsHTML = `
        ...
        pokemon.stats.forEach(stat => {
            detailsHTML += `<li>${stat.stat.name}: ${stat.base_stat}</li>`
        });
        detailsHTML += `</ul>
        <p>Held Items: </p>
        <ul>`;
        pokemon.held_items.forEach(item =>{
            detailsHTML += `<li>${item.item.name}</li>`;
        });
        detailsHTML += `</ul>`;     
        detailsHTML += `<p>Abilities:</p>
        <ul>`;
        pokemon.abilities.forEach(ability => {
            detailsHTML += `<li>${ability.ability.name}</li>`;
        });
        detailsHTML += `</ul>`;
